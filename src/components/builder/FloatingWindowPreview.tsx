...
export function FloatingWindowButton({ config, onClick }: { config: BuilderConfig; onClick?: () => void }) {
  return (
    <Button
      variant="outline"
      size="sm"
...
export function FloatingWindowPreview({ config }: FloatingWindowProps) {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            className="absolute top-20 right-8 w-80 bg-card border border-border rounded-xl shadow-2xl overflow-hidden z-[110]"
            initial={{ opacity: 0, scale: 0.9, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.9, y: 20 }}
            transition={{ type: 'spring', ...motionConfig.spring.medium }}
