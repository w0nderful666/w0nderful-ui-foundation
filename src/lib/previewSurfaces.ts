...
export function getPreviewToolbarClass(config: BuilderConfig): string {
  if (isGlassMode(config)) {
    return 'bg-card/40 backdrop-blur-xl border-b border-white/10'
  }
  if (isDarkTerminalMode(config)) {
...
export function getPreviewPanelClass(config: BuilderConfig): string {
  if (isGlassMode(config)) {
    return 'bg-card/55 backdrop-blur-xl border border-white/15 shadow-[0_18px_60px_hsl(var(--foreground)/0.08)]'
  }
  if (isDarkTerminalMode(config)) {
    return 'bg-black/35 backdrop-blur-md border border-white/10'
  }
  if (isNeonCyberMode(config)) {
    return 'bg-card/70 backdrop-blur-md border border-primary/20 shadow-[0_0_40px_hsl(var(--primary)/0.12)]'
  }
...
export function getPreviewCardClass(config: BuilderConfig): string {
  if (isGlassMode(config)) {
    return 'bg-card/45 backdrop-blur-md border border-white/10 shadow-[0_12px_40px_hsl(var(--foreground)/0.06)]'
  }
  if (isDarkTerminalMode(config)) {
    return 'bg-black/25 backdrop-blur-sm border border-white/10'
  }
  if (isNeonCyberMode(config)) {
    return 'bg-card/60 backdrop-blur-sm border border-primary/15 shadow-[0_0_25px_hsl(var(--primary)/0.08)]'
  }
...
export function getPreviewSurfaceClass(config: BuilderConfig): string {
  if (isGlassMode(config)) {
    return 'bg-card/65 backdrop-blur-2xl border border-white/15'
  }
  if (isDarkTerminalMode(config)) {
    return 'bg-black/45 backdrop-blur-lg border border-white/10'
  }
  if (isNeonCyberMode(config)) {
    return 'bg-card/75 backdrop-blur-lg border border-primary/20'
  }
...
export function getPreviewCodeClass(config: BuilderConfig): string {
  if (isDarkTerminalMode(config)) {
    return 'bg-[#0a0a0a]/90 backdrop-blur-sm border border-white/10'
  }
...
export function getPreviewInputClass(config: BuilderConfig): string {
  if (isGlassMode(config)) {
    return 'bg-card/40 backdrop-blur-sm border border-white/10 focus:border-primary/30'
  }
  if (isDarkTerminalMode(config)) {
    return 'bg-black/30 backdrop-blur-sm border border-white/10 focus:border-primary/30'
  }
...
export function getPreviewGlowClass(config: BuilderConfig): string {
  if (isNeonCyberMode(config)) {
    return 'shadow-[0_0_30px_hsl(var(--primary)/0.2)]'
  }
  if (isGlassMode(config)) {
    return 'shadow-[0_8px_30px_hsl(var(--foreground)/0.08)]'
  }
  return ''
}
...
function isDarkTerminalMode(config: BuilderConfig): boolean {
  ...
  return (
    terminalCardStyles.includes(config.cardStyle as CardStyle) ||
    terminalExperienceStyles.includes(config.experienceStyle as ExperienceStyle)
  )
}
...
export function getPreviewContainerClass(config: BuilderConfig): string {
  if (isGlassMode(config) || isDarkTerminalMode(config)) {
