import { EXTENDED_TEMPLATES as SERIES_SCENE_TEMPLATES } from '../data/seriesExtendedTemplates.js'
import { EXPRESSION_VARIANTS, BODY_NO_HAND_VARIANTS, FRAMING_VARIANTS, HAND_SAFE_VARIANTS } from '../data/seriesVariationPools.js'
...
function pickFromPool(pool, index, rand) {
...
export function generateSeriesPrompts(options = {}) {
  const {
    templateId,
    count = 4,
  ...
  const template = SERIES_SCENE_TEMPLATES.find(t => t.id === templateId)
  if (!template) return []
  ...
  for (let i = 0; i < count; i++) {
    const base = deepClone(template.baseDirector)

    if (characterOverride && typeof characterOverride === 'object') {
