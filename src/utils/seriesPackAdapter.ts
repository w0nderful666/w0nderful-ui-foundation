...
export function saveSeriesAsPack(series: SeriesPromptResult[], packMeta: PackMeta = {}) {
  if (!series || !Array.isArray(series) || series.length === 0) {
    return { success: false, error: 'Series 数据为空', pack: null as any }
  }

  const packs = safeRead<any[]>(STORAGE_KEY, [])
